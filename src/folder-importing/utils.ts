...
export async function getFileTree(
  ...
  const { worker, promise } = runWorker(
    path.join(__dirname, "folder-importing", "worker-dirtree.cjs"),
    filepath
  );
...
> {
  const { worker, promise } = runWorker(
    path.join(__dirname, "folder-importing", "worker-clean-dirtree.cjs"),
    tree
  );
...
export async function flattenFileTreeForCheckbox(
  ...
  const { worker, promise } = runWorker(
    path.join(
      __dirname,
      "folder-importing",
      "worker-flatten-tree-for-checkbox.cjs"
    ),
    fileTree
  );
...
> {
  const { worker, promise } = runWorker(
    path.join(__dirname, "folder-importing", "worker-remove-deselected.cjs"),
    { tree, user_selection }
  );
  const timeout = createTimeoutPromise(2000);
  const result = await Promise.race([promise, timeout]);
